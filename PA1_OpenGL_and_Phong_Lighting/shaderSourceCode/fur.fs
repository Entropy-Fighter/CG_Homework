...
void main(){
    vec3 surface_to_light = normalize(light_pos - new_pos);
    float distance_to_light = length(light_pos - new_pos);
    ...
    // diffuse light
    vec3 normalized_normal = normalize(new_normal);
    vec3 light_direction = normalize(light_pos - new_pos);
    float diff = max(dot(normalized_normal, light_direction), 0.0);
    vec3 diffuse = diff * light_color;

    // specular light
    float specular_strength = 0.5;
    vec3 view_direction = normalize(view_pos - new_pos);
    vec3 reflect_direction = reflect(-light_direction, normalized_normal);
    float spec = pow(max(dot(view_direction, reflect_direction), 0.0), 32);
    vec3 specular = specular_strength * spec * light_color;

    // spotlight (soft edges)
    float theta = dot(light_direction, normalize(-cone_direction));
